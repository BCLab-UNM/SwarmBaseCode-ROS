...
int imageLocX = MAP_ORIGIN_X;
int imageLocY = MAP_ORIGIN_Y;
int obstacleMode = 0;
 ...
 */
int obstacleCost = 20; //amount to adjust map for found obstacles 
...
int pheromoneCost = -10; //amount to adjust map for the deposited pheromone
...
int main(int argc, char** argv) {
    ...
    targetSubscriber = mNH.subscribe((publishedName + "/targets"), 2, targetHandler);
    locationSubscriber = mNH.subscribe((publishedName + "/location_image"), 10, imageLocHandler);
    pheromoneSubscriber = mNH.subscribe((publishedName + "/pheromone"), 1, pheromoneHandler);
    ...
    batterySubscriber = mNH.subscribe((publishedName + "/battery"), 1, batteryHandler);
    chargingStateSubscriber = mNH.subscribe((publishedName + "/chargingState"), 1, chargingStateHandler);

    publishTimer = mNH.createTimer(ros::Duration(1.0), publishMap);
    decayTimer = mNH.createTimer(ros::Duration(mapDecayTime), decayMap); 

    ros::ServiceServer mapHarvestService = mNH.advertiseService((publishedName + "/mapHarvestSrv"), mapHarvestHandler); // robot specific service
...
void imageLocHandler(const geometry_msgs::Pose2D::ConstPtr& message) {
    // no conversion necessary.  this is the location on the raw map, with origin in upper left corner
    currentHeading = (float) message->theta;
    imageLocX = (int) message->x; 
    imageLocY = (int) message->y; 
}
...
void publishMap(const ros::TimerEvent& e) {    
    // mark obstacles, but ignore the far out caution obstacle till it gets closer
...
void updateMap(int deltaX, int deltaY, int targetSize, int cost) {
    int splatLocX = deltaX + imageLocX;
    int splatLocY = deltaY + imageLocY;
    
    //request world state module to update master map
    mapUpdateMessage.request.X = splatLocX;
    mapUpdateMessage.request.Y = splatLocY;
