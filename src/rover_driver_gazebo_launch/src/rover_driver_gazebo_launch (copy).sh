...
{                                # Caution: Max return value = 255.
  ...
  inputValue=$(bc <<< "$1>0")
  if [ "$inputValue" == "1" ]              # If non-negative,
  then                           #
...
nameCounter=0
for (( i=2; i<=$((numberOfRovers+1)); i++ ));do	
  if [ -z "${arguments[$i]}" ];then
   echo "Rover name missing, add a rover:"
    while true;do
      read roverName
        if [ -z "$roverName" ];then
          ...
          roverNameArray[$nameCounter]="$roverName"
    ...
    #DEBUG echo "This is a good one: ${arguments[$i]}"
...
echo ${#roverNameArray[@]}
counter=$nameCounter
while [ -n ${arguments[$numberOfRovers+$counter]}//[0-9]/} ] && [ -z ${arguments[$numberOfRovers+$counter]} ];do
  if [ -z ${arguments[$numberOfRovers+$counter]} ] || [ ${arguments[$numberOfRovers+$counter]} -eq ${arguments[$numberOfRovers+$counter]} 2>/dev/null ];then
    break
  else
    echo "Uh oh"
    echo "It looks like you wanted to spawn $numberOfRovers rovers, but you wrote more than $numberOfRovers names down"
...
for ((i = 0; i < $numberOfRovers; i++));do
  ...
  echo "Number of Rovers $numberOfRovers"
  echo "Rovers being spawned: ${roverNameArray[@]}"
  echo "Current Rover: ${roverNameArray[$i]}"
...
cursorPosition=${arguments[$numberOfRovers+3]}
if [ -z "$cursorPosition" ] || [[ -n ${cursorPosition//[0-9]/} ]];then
  tagDist
elif [[ $cursorPosition -eq 1 ]];then
  echo "Uniform"
  uniformDist=true
elif [[ $cursorPosition -eq 2 ]];then
  echo "Clustered"
  clusteredDist=true
elif [[ $cursorPosition -eq 3 ]];then
  echo "PowerLaw"
  powerLawDist=true
fi

#TODO: This is redundant.  this was already checked up above.  
permissions 'add_rovers'
permissions 'randomlocations'
permissions 'rover_driver_node_launch'

#TODO: Maybe remove these at the end of the script to keep the folder clean
rm tmp.txt
...
#Checks to see if the rover they choose exists in the ~/rover_misc_workspace/src/gazebo/models dir.

for ((i = 0; i < $numberOfRovers; i++));do
  echo "Searching for ${roverNameArray[$i]} in ~/rover_misc_workspace/src/gazebo/models/"
  echo
  if [ ! -d ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}" ];then
    #Directory doesn't exist. Asks if you would like to change the name of the rover or add the model files manually
    echo "Would you like to change the rovers name you wish to spawn first [0] or add ${roverNameArray[$i]} model files manually to the directory ~/rover_misc_workspace/src/gazebo/models/ [1]?"
    while true;do
      read choice
    #Choose to rename the rover
      if [ "$choice" == "0" ] && [ ! -d ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}" ];then
        echo "What is the new name for the rover?"
      #Waits for vaild input for name. Will check if that rover exists. If you do not have any rovers in the directory ~/rover_misc_workspace/src/gazebo/models/ you will need to add them manually
        while true;do
        read roverName
          if [ ! -d ~/rover_misc_workspace/src/gazebo/models/"$roverName" ];then
            echo "Model doesn't exist. Choose a model that exists in the directory ~/rover_misc_workspace/src/gazebo/models/"
            echo 
            echo
          else
            echo
    ...
    #Choose to manually add the selected rover files into echo ~/rover_misc_workspace/src/gazebo/models/
      elif [ "$choice" == "1" ] && [ ! -d ~/rover_misc_workspace/src/gazebo/models/"$roverName" ];then
        echo "Is $roverName model files in the directory ~/rover_misc_workspace/src/gazebo/models/? [y]"
        while true;do
          read response
        #response is yes but the files cannot be located
          if [ "$response" == "y" ] && [ ! -d ~/rover_misc_workspace/src/gazebo/models/"$roverName" ];then
            echo 
            echo "Make sure that you inserted the model into the correct directory."
            echo
        #response is yes and the files can be located
          elif [ "$response" == "y" ] && [ -d ~/rover_misc_workspace/src/gazebo/models/"$roverName" ];then
            echo
            echo "$roverName found!"
            roverNameArray[$i]=$roverName
            echo
    ...
    #Garbage response
      elif [ ! -d ~/rover_misc_workspace/src/gazebo/models/"$roverName" ];then
...
#      if [ $i == "0" ];then
#        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x 0 -y 0 -z 0 -R 0 -P 0 -Y 0;
#        echo "0 0" >> tmp.txt
#        echo
#      elif [ $i == "1" ];then
#        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x 0 -y 1.0 -z 0 -R 0 -P 0 -Y -1.55;
#        echo "0 1.0" >> tmp.txt
#        echo
#      elif [ $i == "2" ];then
#        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x -1.0 -y 0 -z 0 -R 0 -P 0 -Y 3.15;
#        echo "-1.0 0" >> tmp.txt
#        echo
#      elif [ $i == "3" ];then
#        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x 0 -y -1.0 -z 0 -R 0 -P 0 -Y #3.15;
#        echo "0 -1.0" >> tmp.txt
#        echo
#      elif [ $i == "4" ];then
#        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x 0 -y -1.0 -z 0 -R 0 -P 0 -Y 40;
#        echo "0 -1.0" >> tmp.txt
#        echo
#      fi
#    done
...
#Came back to this...There is almost certainly a more elegant solution
  for ((i = 0; i < $numberOfRovers; i++));do
    ...
    gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x $x -y ${yCoordArray[$yCounter]} -z 0 -R 0 -P 0 -Y 0;
      ...
      if [ "$originOccupied" = true ];then
        ...
        while true;do
          ...
          if [ "$repeats" -eq "0" ] ;then
            ...
            echo "X: $randomXCoord"
            echo "Y: $randomYCoord"
            gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y 0;
        ...
        while true; do
          ...
          if [ "$repeats" -eq "0"  ] ;then
            ...
            gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/"${roverNameArray[$i]}"/model.sdf -m "${roverNameArray[$i]}" -x "$selectedXCoord" -y "$selectedYCoord" -z 0 -R 0 -P 0 -Y 0;
  ...
  for ((i = 0; i < "$numberOfTags"; i++)); do
    while true;do
     ...
     if [ "$repeats" -eq "0" ] ;then
        ...
        echo "X: $randomXCoord"
        echo "Y: $randomYCoord"
        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/at"$i"/model.sdf -m at"$i" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y $RANDOM;
        #echo "$randomXCoord" "$randomYCoord" >> tmp.txt
        echo
        break
      elif [ "$repeats" -gt "0" ] ;then
  ...
  for ((i = 0; i < 4; i++)); do
    while true;do
      ...
      if [ "$repeats" -eq "0" ] ;then
        ...
        echo "X: $randomXCoord"
        echo "Y: $randomYCoord" 
        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/atags64_"$i"/model.sdf -m atags64_"$i" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y $RANDOM;
       # echo "$randomXCoord" "$randomYCoord" >> tmp.txt
        echo
        break
      elif [ "$repeats" -gt "0" ];then
...
#for DEBUG      echo "Too Close Y: $tooCloseY"
      ...
      if [ "$repeats" -eq "0" ];then
        ...
        echo "Y: $randomYCoord"
        #Spawns 1 pile of 64 tags
        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/atags16_"$i"/model.sdf -m atags16_"$i" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y $RANDOM;
       # echo "$randomXCoord" "$randomYCoord" >> tmp.txt
        echo
        break
      elif [ "$repeats" -gt "0" ] ;then
...
#for DEBUG      echo "Too Close Y: $tooCloseY"
      ...
      if [ "$repeats" -eq "0" ];then
        ...
        echo "Y: $randomYCoord"
        #Spawns 1 pile of 64 tags
        gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/atags4_"$i"/model.sdf -m atags4_"$i" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y $RANDOM;
        #echo "$randomXCoord" "$randomYCoord" >> tmp.txt
        echo
        break
      elif [ "$repeats" -gt "0" ] ;then
...
#for DEBUG      echo "Too Close Y: $tooCloseY"
    ...
    if [ "$repeats" -eq "0" ] ;then
      ...
      echo "X: $randomXCoord"
      echo "Y: $randomYCoord"
      gzfactory spawn -f ~/rover_misc_workspace/src/gazebo/models/swarmie_obstacle/model.sdf -m swarmie_obstacle_"$i" -x "$randomXCoord" -y "$randomYCoord" -z 0 -R 0 -P 0 -Y 0;
     # echo "$randomXCoord" "$randomYCoord" >> tmp.txt
      echo
      break
    elif [ "$repeats" -gt "0" ] ;then
