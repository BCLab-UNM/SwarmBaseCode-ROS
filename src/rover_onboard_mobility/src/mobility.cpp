...
int startingDebugCounter = 0;
int turningDebugCounter = 0;
int walkingDebugCounter = 0;
...
void move() {
	...
	// debug messages for stuck code
	if (finishedTurning && finishedWalking) {
		turningDebugCounter = 0;
		walkingDebugCounter = 0;
		startingDebugCounter++;
		//cout << "Start of turn/walk cycle (" << startingDebugCounter << ")." << endl;

		if (startingDebugCounter >= 100) {
			cout << " ***** This might be our stuck state!  Forcing a turn! *****" << endl;
			finishedTurning = false;
			finishedWalking = true;
                	walkingTime = 0;
            		overshootAttempts = 0;
			startingDebugCounter = 0;
		}

	}
	else if (!finishedTurning && !finishedWalking) {
		cout << "NOT done turning and NOT done walking." << realHeading << ", " << desiredHeading << endl;
		cout << " ***** This might be our stuck state!  Resetting! *****" << endl;
		...
		startingDebugCounter = 0;
		walkingDebugCounter++;
		//if ( (walkingDebugCounter == 1) || (walkingDebugCounter >= 20) ) {
			//cout << "Walking (" << walkingDebugCounter << ") towards " << realHeading << endl;
		//}

		if (walkingDebugCounter >= 80) {
			cout << " ***** This might be our stuck state!  Resetting! *****" << endl;
			finishedTurning = true;
			finishedWalking = true;
                	walkingTime = 0;
		...
		startingDebugCounter = 0;
		turningDebugCounter++;
		//if ( (turningDebugCounter == 1) || (turningDebugCounter >= 100) ) {
			//cout << "Turning (" << turningDebugCounter << ") from " << realHeading << " to " << desiredHeading << endl;
		//}

		if (turningDebugCounter >= 200) {
			cout << " ***** This might be our stuck state!  Resetting! *****" << endl;
			finishedTurning = true;
			finishedWalking = true;
                	walkingTime = 0;
		...
		cout << "Invalid state. " << realHeading << ", " << desiredHeading << endl;
	}
	...
	if ( (abs(realPositionX) > digitalFenceSize) || (abs(realPositionY) > digitalFenceSize) ) {
            overshootAttempts = 0;// TODO: this probably does not need to be here, now that it is in the proper place.
		...
		if (!isOutsideDigitalFence) { 
			...
			totalDigitalFenceCount++; 
	    		...
	    		travelHeading = travelHeading + 30; // shift travel heading for later after fence is cleared
            		...
            		cout << "New heading to get back inside digital fence: " << desiredHeading << endl;
    			...
    			finishedWalking = true; // attempt to remove race condition
    			walkingTime = 0; // attempt to remove race condition
    			finishedTurning = false; // attempt to remove race condition
	...
	if (isHeadingToTarget && !isOutsideDigitalFence) {
		...
		//searchCorrelation = uninformedCorr;
		    ...
		    targetHeading = rad2deg(atan2(targetPosY - realPositionY, targetPosX - realPositionX)); 
...
// TODO: That entire atan2 and translation section is used twice in this file.  Turn it into a helper subroutine
            	    ...
            	    //cout << "Calculated heading towards target: " << desiredHeading << endl;

		    //if (startingDebugCounter == 1) {
			//cout << "Calculated heading to target " << targetPosX << ", " << targetPosY << " is " << desiredHeading << endl;
		    //}

		    //cout << "Stuck state? " << finishedTurning << ", " << finishedWalking << ", " 
			 			    //<< walkingTime << ", " << desiredHeading << endl;
        ...
        if (finishedTurning && finishedWalking && !isOutsideDigitalFence) { 
            ...
            if (isGoingHome) {
            	...
            	desiredHeading = normalizeHeading(desiredHeading); 
            	//cout << "Calculated heading towards home is " << desiredHeading << endl;
                //searchCorrelation = uninformedCorr;
		...
		//cout << "Calculated heading to target " << targetPosX << ", " << targetPosY << " is " << desiredHeading << endl;
                ...
                lookForPheromones();
                desiredHeading = pheromoneHeading;
            	//cout << "Calculated heading towards pheromone trail is " << desiredHeading << endl;
                //searchCorrelation = uninformedCorr;
            } else if (isInvestigatingArea) {
                //cout << "Investigating Area" << endl;
                desiredHeading = realHeading;  // TODO: This may be where it's getting stuck!!!
		if (arrivedViaPheremonesOrViaFidelity){
			...
			// need to force an uninformed search (nearly straight)
            	...
            	//cout << "Setting search correlation to " << searchCorrelation << " to randomize heading." << endl;

            } else if (isTraveling) {
                //cout << "Still Traveling" << endl;
                desiredHeading = travelHeading;  
            		//cout << "Travel mode towards heading: " << desiredHeading << endl;
                //searchCorrelation = 180;
            } else {
		//cout << "Now what?" << endl;
	    }
		...
		desiredHeading = normalRandom(desiredHeading, searchCorrelation); 
            	//cout << "Heading randomization due to search correlation: " << desiredHeading << endl;
	    }

	    if (isFollowingPheromones) {
                desiredHeading = pheromoneHeading;
            	//cout << "New pheromone heading is " << desiredHeading << endl;
            }

	    if (isHeadingToTarget) {
                desiredHeading = targetHeading;
            	//cout << "New target heading is " << desiredHeading << endl;
            }

            desiredHeading = normalizeHeading(desiredHeading); 
            //cout << "Setting up a new turn towards heading: " << desiredHeading << endl;
		...
		finishedWalking = false; // attempt to remove race condition
		finishedTurning = true; // attempt to remove race condition
		...
		//cout << "Done turning." << endl;
                ...
                if (differentialDirection != turnDirection) {
            	    ...
            	    desiredHeading = normalizeHeading(desiredHeading); 
            	    cout << "Overshot our turn. Turn heading from other direction: " << desiredHeading << endl;
...
// TODO: does the above line get setRotationSpeed to get called somehow?
    ...
    } // end of no obstacle was detected
...
// TODO: This section is never getting called because the caller never calls this when in mode 1 or 9, thus robot sticks in slow speed.
	    ...
	    // world keeps track of obstacle calls
        ...
        } // end if (finishedObstacleAction)
// TODO: need to understand this logic.  Is it possible to get here and never get finished, thus causing robot to get stuck
	else {
		//cout << "In avoidObstacle(), but finishedObstacleAction is false.  Is this our stuck state?" << endl;
	}
...
void stuckChecker(const ros::TimerEvent&) {
    ...
    if ( (currentMode == 2 || currentMode == 3) && !isCharging) { //If robot is in auto mode and not charging
...
void batteryHandler(const std_msgs::Float32& message) {
   /* //Excessive amount of debug statements.
    ...
    if (batteryLevel >= chargeLeave) {//if the battery is full, go out and explore
        ...
        if (isCharging) {
            cout << "Battery charge is complete." << endl;
