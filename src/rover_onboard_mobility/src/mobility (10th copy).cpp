...
void move() {
	...
	if (finishedTurning && finishedWalking) {
		...
		cout << "Start of walk/turn logic (" << startingDebugCounter << "). real: " << realHeading << ", desired: " << desiredHeading << endl;

		if (startingDebugCounter >= 80) {
			cout << " ***** This might be the stuck state!  Forcing a turn! *****" << endl;
		...
		cout << " ***** This might be the stuck state!  Resetting! *****" << endl;
		...
		if ( (walkingDebugCounter == 1) || (walkingDebugCounter >= 20) ) {
			cout << "Walking (" << walkingDebugCounter << ") " << realHeading << ", " << desiredHeading << endl;
		}
		...
		if (!isOutsideDigitalFence) { 
			...
			totalDigitalFenceCount++; 
	    		...
	    		travelHeading = travelHeading + 30; // shift travel heading for later after fence is cleared
            		...
            		cout << "1) Setting desired heading to " << desiredHeading << endl;
...
// TODO: That entire atan2 and translation section is used twice in this file.  Turn it into a helper subroutine
            		...
            		cout << "2) Setting desired heading to " << desiredHeading << endl;
        ...
        if (finishedTurning && finishedWalking && !isOutsideDigitalFence) { 
            ...
            if (isGoingHome) {
            	...
            	desiredHeading = normalizeHeading(desiredHeading); 
            		cout << "3) Setting desired heading to " << desiredHeading << endl;
                //searchCorrelation = uninformedCorr;
            } else if (isFollowingPheromones) {
                //cout << "Following Pheromone Trail" << endl;
                lookForPheromones();
                desiredHeading = pheromoneHeading;
            		cout << "4) Setting desired heading to " << desiredHeading << endl;
                //searchCorrelation = uninformedCorr;
                ...
                desiredHeading = realHeading;  // TODO: This may be where it's getting stuck!!!
            		cout << "5) Setting desired heading to " << desiredHeading << endl;
		if (arrivedViaPheremonesOrViaFidelity){
			...
			// need to force an uninformed search (nearly straight)
            		...
            		cout << "Setting search correlation to " << searchCorrelation << endl;

            } else if (isTraveling) {
                //cout << "Still Traveling" << endl;
                desiredHeading = travelHeading;  
            		cout << "6) Setting desired heading to " << desiredHeading << endl;
                //searchCorrelation = 180;
            } else {
		//cout << "Now what?" << endl;
	    }
	    ...
	    if (isFollowingPheromones) {
            ...
            desiredHeading = normalizeHeading(desiredHeading); 
            cout << "7) Setting desired heading to " << desiredHeading << endl;
            headingDifferential = angleBetweenHeadings(realHeading, desiredHeading);
            turnDirection = headingDifferential / abs(headingDifferential);
