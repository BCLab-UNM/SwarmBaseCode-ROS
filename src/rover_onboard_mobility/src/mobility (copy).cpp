...
float turnAccuracy = 10; //degree tolerance needed to "match" new heading
float digitalFenceSize = 15; //how many meters the robot is allowed to travel in x or y direction, square fence
...
int main(int argc, char **argv) {
    ...
    batterySubscriber = mNH.subscribe((publishedName + "/battery"), 10, batteryHandler); //Reads the battery level being published by the battery module
    ...
    mobilityPublish = mNH.advertise<geometry_msgs::Twist>((publishedName + "/mobility"), 100, false);
    pheromonePublish = mNH.advertise<std_msgs::Bool>((publishedName + "/pheromone"), 10, true);
    chargingStatePublisher = mNH.advertise<std_msgs::Bool>((publishedName + "/chargingState"), 10, true); //publishes whether or not the rover is currently at home charging its batteries.
    harvestedPublish = mNH.advertise<std_msgs::UInt64>((publishedName + "/harvested"), 10, true); //publishes the targets harvested per rover
    stateMachinePublish = mNH.advertise<std_msgs::String>((publishedName + "/state_machine"), 10, true); //publishes the current state of the state machine, in human readable form
...
void lookForPheromones() {
	...
	if (stateMachineState == STATE_MACHINE_TRAVEL) {
    		trackPheromoneMessage.request.coneAngle = 40;
// TODO: This is too simplistic.  Too wide a cone and we get to the end of a trail and turn around and follow it backwards.  Too narrow of a cone and we can't follow a trail around obstacles.
	}
	else {
